...
void ScanFiner::updateParams() {
  ...
  nh_local_.param<std::string>("scanner_frame", p_scanner_frame_, "base_scan");
...
void ScanFiner::globalcostmapCallback(const nav_msgs::OccupancyGrid::ConstPtr& costmap_input){
  ...
  for (int j=0; j<global_height_; j++){
    <vector<int>> costmap_width_temp_(global_width_,0); //should be confirmed - temporally allocated vetor from for loop used as a data
    for (int i=0; i<global_width_; i++){
      costmap_width_temp_.push_back(costmap_points_[i+j*global_width_]);
...
void ScanFiner::maskingScan(){
  for (Point& point_ : temp_points_){
    int mask_i = point_.x/global_resolution_;
    int mask_j = point_.y/global_resolution_;
...
void ScanFiner::publishFinedScan(const sensor_msgs::LaserScan::ConstPtr& scan) {
  ...
  fined_scan_.header.frame_id = p_world_frame_;
